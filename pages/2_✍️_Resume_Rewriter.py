...
# Client simple compatible OpenAI pour serveurs locaux (ex: Ollama)
class _SimpleChoice:
...
def _clean_jsonish_text(text: str) -> str:
    """Normalize quotes and remove trailing commas common in LLM outputs."""
...
def _extract_json_block(text: str) -> str | None:
    """Extract JSON/code block or first bracketed segment from text."""
    if not text:
        return None
    import re as _re
    m = _re.search(r"```json\s*(.*?)\s*```", text, _re.DOTALL | _re.IGNORECASE)
    if m:
        return m.group(1).strip()
    m = _re.search(r"```\s*(.*?)\s*```", text, _re.DOTALL)
    if m:
        return m.group(1).strip()
    # Fallback to bracketed slice
    starts = [("[", text.find("[")), ("{", text.find("{"))]
    starts = [(ch, idx) for ch, idx in starts if idx != -1]
    if not starts:
        return None
    ch, start = sorted(starts, key=lambda x: x[1])[0]
    closing = "]" if ch == "[" else "}"
...
def safe_json_loads(text: str, expect: str = "any"):
    """Try strict JSON, then Python literal, then bracket extract. Return sensible defaults."""
    import json as _json
    block = _extract_json_block(text) or text
    block = _clean_jsonish_text(block)
    try:
    ...
    try:
        ...
        pass
    try:
        b2 = _extract_json_block(block)
        if b2 and b2 != block:
            return _json.loads(b2)
    except Exception:
        pass
    if expect == 'array':
        return []
...
Return as JSON array (STRICT JSON, no markdown, no text):
[
  ...
  ...
    ...
    try:
        model_name = st.session_state.get('rewriter_llm_model', "tngtech/deepseek-r1t2-chimera:free")
        extra_headers = st.session_state.get('rewriter_llm_extra_headers', {
        ...
        return safe_json_loads(response_text, expect='array') or []
...
Return as JSON (STRICT JSON, no markdown, no text):
{{
  ...
  ...
    ...
    try:
        ...
        response_text = completion.choices[0].message.content
        return safe_json_loads(response_text, expect='object') or {}
    except Exception as e:
...
def generate_optimized_resume_docx(original_text: str, optimized_data: dict) -> BytesIO:
    ...
    if optimized_data.get('deepseek_enabled') and optimized_data.get('deepseek_optimized'):
        ...
        # Education
        if optimized_sections.get('education'):
            doc.add_heading('EDUCATION', 1)
            education_data = optimized_sections['education']
            if isinstance(education_data, list):
                for e in education_data:
                    doc.add_paragraph(e)
            else:
                # Split lines if provided as a single string
                lines = [l.strip() for l in str(education_data).split('\n') if l.strip()]
                if len(lines) > 1:
                    for l in lines:
                        doc.add_paragraph(l)
                else:
                    doc.add_paragraph(str(education_data))
            doc.add_paragraph()
    else:
...
def generate_optimized_resume_pdf(original_text: str, optimized_data: dict) -> BytesIO:
    ...
    if optimized_data.get('deepseek_enabled') and optimized_data.get('deepseek_optimized'):
        ...
        if optimized_sections.get('summary'):
            ...
            story.append(Paragraph(optimized_sections['summary'], body_style))
            story.append(Spacer(1, 0.2*inch))

        # Experience
        exp = optimized_sections.get('experience')
        if exp:
                ...
                for entry in exp:
                    if isinstance(entry, dict):
                        ...
                        if desc:
                            ...
                            if lines:
                                list_items = [ListItem(Paragraph(l, body_style)) for l in lines]
                                story.append(ListFlowable(list_items, bulletType='bullet'))
                    elif isinstance(entry, str):
                        items.append(ListItem(Paragraph(entry, body_style)))
            elif isinstance(exp, str):
                # Split single string into bullet lines
                lines = [l.strip(' •-*–—') for l in exp.split('\n') if l.strip()]
                items = [ListItem(Paragraph(l, body_style)) for l in lines]
            if items:
                story.append(ListFlowable(items, bulletType='bullet'))
            story.append(Spacer(1, 0.2*inch))

        # Skills
        skills = optimized_sections.get('skills')
        if skills:
            story.append(Paragraph("SKILLS", heading_style))
            skill_items = []
            if isinstance(skills, list):
                skill_items = [ListItem(Paragraph(s, body_style)) for s in skills if s]
            elif isinstance(skills, str):
                # Split by newline or bullets or commas when appropriate
                if '\n' in skills:
                    parts = [p.strip(' •-*–—') for p in skills.split('\n') if p.strip()]
                else:
                    parts = [p.strip() for p in skills.split(',') if p.strip()]
                skill_items = [ListItem(Paragraph(p, body_style)) for p in parts]
            if skill_items:
                story.append(ListFlowable(skill_items, bulletType='bullet'))
            story.append(Spacer(1, 0.2*inch))

        # Education
        education = optimized_sections.get('education')
        if education:
            story.append(Paragraph("EDUCATION", heading_style))
            if isinstance(education, list):
                edu_items = [ListItem(Paragraph(e, body_style)) for e in education if e]
                story.append(ListFlowable(edu_items, bulletType='bullet'))
            else:
                # Split by lines if multi-line string
                lines = [l.strip() for l in str(education).split('\n') if l.strip()]
                if len(lines) > 1:
                    story.append(ListFlowable([ListItem(Paragraph(l, body_style)) for l in lines], bulletType='bullet'))
                else:
                    story.append(Paragraph(str(education), body_style))
            story.append(Spacer(1, 0.2*inch))
    else:
...
def main():
    ...
    with st.sidebar:
        ...
        ai_model = st.radio(
            ...
            options=["DeepSeek R1 (Recommandé)", "Local LLM (Ollama)", "Analyse Standard"],
            index=0,
            help="Choisissez le moteur IA pour générer des suggestions personnalisées"
        )
        ...
        if ai_model == "DeepSeek R1 (Recommandé)":
            ...
            if effective_key:
                ...
                # Utiliser le client simple pour éviter les problèmes de proxies de la lib openai
                st.session_state['rewriter_deepseek_client'] = SimpleOpenAIClient(
                    base_url="https://openrouter.ai/api/v1",
                    api_key=effective_key,
                )
                st.session_state['rewriter_llm_model'] = "tngtech/deepseek-r1t2-chimera:free"
                st.session_state['rewriter_llm_extra_headers'] = {
                    "HTTP-Referer": "https://github.com/FEDI-HASSINE/cv-ai-",
                    "X-Title": "UtopiaHire Resume Rewriter",
                }
            ...
            base_url = st.text_input(
                ...
                key="rewriter_ollama_base_url"
            )
            model_name = st.text_input(
                "Model Name",
                value="qwen2.5:1.5b-instruct",
                help="Nom du modèle Ollama (ex: qwen2.5:1.5b-instruct, phi3:mini, mistral:7b-instruct)",
                key="rewriter_ollama_model"
            )
            api_key = st.text_input(
                "API Key",
                value="ollama",
                help="Valeur arbitraire exigée par certains clients; pas utilisée par Ollama",
                key="rewriter_ollama_api_key"
            )
            if base_url and model_name:
                try:
                    ...
                    st.session_state['rewriter_llm_model'] = model_name
                    st.session_state['rewriter_llm_extra_headers'] = {}
                    st.session_state['use_rewriter_deepseek'] = True
    ...
    if uploaded_file is not None:
        ...
        if 'rewrite' in st.session_state:
            ...
            with tab1:
                ...
                if rewrite.get('deepseek_enabled') and rewrite.get('deepseek_weak_phrases'):
                    st.info(f"AI-Detected: {len(rewrite['deepseek_weak_phrases'])} weak phrases")
                    for i, phrase in enumerate(rewrite['deepseek_weak_phrases'][:10], 1):
                        with st.expander(f"❌ Phrase {i}: \"{phrase.get('original', 'N/A')}\"", expanded=(i <= 3)):
                            ...
                            st.markdown("**✅ Stronger alternative:**")
            ...
            with tab2:
                ...
                if rewrite.get('deepseek_enabled') and rewrite.get('deepseek_action_verbs'):
                    st.info("AI-Contextualized action verbs")
                    verbs = rewrite['deepseek_action_verbs']
                    verb_categories = list(verbs.keys())[:4] if isinstance(verbs, dict) else []
                    if verb_categories:
                        tabs = st.tabs(verb_categories)
                        for inner_tab, category in zip(tabs, verb_categories):
                            with inner_tab:
                                verb_list = verbs.get(category, [])
                        ...
                        st.info("No action verb categories found in AI output.")
                else:
            ...
            with tab3:
                ...
                if rewrite.get('deepseek_enabled') and rewrite.get('deepseek_quantification'):
                    for i, quant in enumerate(rewrite['deepseek_quantification'][:8], 1):
            ...
            with tab4:
                ...
                if rewrite.get('deepseek_enabled') and rewrite.get('deepseek_formatting'):
            ...
            # Déplacer les recommandations IA vers la section "Priority Recommendations" ci-dessous
